...
const EventSheet = () => {
  ...
  const handleSecondaryOpenAndReset = usePassport(
    ...
    const displayedEvent = filteredEvents.find(event => event.id === currentEventId);
  ...
  return (
    ...
    >
      <SheetContent hideOverlay side="right" className="z-[40000] px-0 cursor-default">
        {displayedEvent ? (
            <>
            <SheetOverlay style={{
                backgroundColor: "rgba(0, 0, 0, 0.1)",
                zIndex: -40000,
              }} asChild className="bg-transparent z-[-4000]" data-state="closed" />
                <div className="space-y-3">
                  <div className="px-6 space-y-3">
                    <div>
                      <p className="text-gray-500 text-sm">{displayedEvent.date}</p>
