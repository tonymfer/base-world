export const pin = `<svg width="36.6" height="55" viewBox="0 0 146 219" fill="none" xmlns="http://www.w3.org/2000/svg">
<g clip-path="url(#clip0_155_796)">
    <path
        d="M73 219C73 218 146 113.317 146 73C146 32.6832 113.317 0 73 0C32.6832 0 0 32.6832 0 73C0 113.317 73 219 73 219Z"
        fill="currentColor" />
    <path
        d="M73.323 123.729C101.617 123.729 124.553 100.832 124.553 72.5875C124.553 44.343 101.617 21.4463 73.323 21.4463C46.4795 21.4463 24.4581 42.0558 22.271 68.2887H89.9859V76.8864H22.271C24.4581 103.119 46.4795 123.729 73.323 123.729Z"
        fill="white" />
</g>
<defs>
    <clipPath id="clip0_155_796">
        <rect width="146" height="219" fill="white" />
    </clipPath>
</defs>
</svg>`;
